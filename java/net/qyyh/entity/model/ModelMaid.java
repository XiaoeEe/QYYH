...
public class ModelMaid extends ModelBiped{
    ...
    public ModelMaid(float modelSize)
    {
    ...
    public void setRotationAngles(float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scaleFactor, Entity entityIn)
    {
        ...
        if(entityIn.getDataManager().get(EntityMaid.IS_STOP)){
        	this.bipedRightArm.rotateAngleX = -0.61F;
        	this.bipedRightArm.rotateAngleY = -0.35F;
        	this.bipedRightArm.rotateAngleZ = -0.08F;
	        this.bipedLeftArm.rotateAngleX = -0.61F;
	        this.bipedLeftArm.rotateAngleY = 0.35F;
	        this.bipedLeftArm.rotateAngleZ = 0.08F;
        }
        if (entityIn.isSneaking())
        {
